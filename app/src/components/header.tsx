...
function Header(): JSX.Element {
  ...
  // ヘッダーテキストの設定
  const pageName =
    path.slice(0, 1 - path.length).toUpperCase() + path.slice(1).toLowerCase()
  const linkPages = Array.from(PATH_MAP.keys()).filter(
    (path) => path !== pageName
  )
  // アイコンの設定
  let icon = ''
    ...
    <header className='mu-header main-content'>
      ...
      <div className='mu-flex mu-flex__horizon mu-flex__center mu-header__secondary'>
        <div className='mu-header__secondary-text'>
          <Link href={PATH_MAP.get(linkPages[0]) || 'NotFound'}>
            <a>{linkPages[0]}</a>
          </Link>
        </div>
        <div className='mu-header__secondary-text'>
          <Link href={PATH_MAP.get(linkPages[1]) || 'NotFound'}>
            <a>{linkPages[1]}</a>
          </Link>
        </div>
      </div>
